...
for(let i = 0;i < cardDecks.length;i++) {
    if(i === 1) {
        for(let j = 0;j < cardDecks[2];j++) {
            pivotCards[i] = cardDecks[2][j]
        }
    } else if(i === 2) {
        for(let j = 0;j < cardDecks[1];j++) {
            pivotCards[i] = cardDecks[1][j]
        }
    } else {
        for(let j = 0;j < cardDecks[i];j++) {
            pivotCards[i] = cardDecks[i][j]
        }
    }
}
function loadCardEls() {
    cardEls.forEach((cardEl, index)=>{
        cardEl.textContent = pivotCards[index]
    })
}